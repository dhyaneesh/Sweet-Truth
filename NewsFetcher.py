...
#General variables
#strKey = "ee939f96c4cfe0db1ab6f1c956a71199"
#strKey = "ce73cd9f82dcc1c02e99ac90c1a74771"
#strKey = "6e8f838cd9ed33adb8f77ee37d8cc404"
#lstStrKey = ["ee939f96c4cfe0db1ab6f1c956a71199","ce73cd9f82dcc1c02e99ac90c1a74771","6e8f838cd9ed33adb8f77ee37d8cc404"]
#strKey = "6dae153cf72ab1f862a878e9012434e1"
#strKey = "ed8c108adce801e378bfea9c05245e8a"
strKey = "082e61f3ef2aa34a3f5e34329914c726"
lstStrKey = ["082e61f3ef2aa34a3f5e34329914c726","ed8c108adce801e378bfea9c05245e8a","6dae153cf72ab1f862a878e9012434e1"]
#lstStrKey = ["6dae153cf72ab1f862a878e9012434e1","",""]
strCountry = "in"
strSearchQuery = "happy"
strTopHeadlinesURL = "https://gnews.io/api/v4/top-headlines?lang=en&country=" + strCountry + "&max=100&token=" + strKey
strSearchQueryURL = "https://gnews.io/api/v4/search?sortby=publishedAt&q="
lstMasterNews = []
lstMasterNewsTitle = []
lstGoodNews = []
...
def clean(raw):
    ...
    result = re.sub("<[o][^>]*>(.+?)</[o]*>", 'Link.', result)
    result = re.sub("<[l][^>]*>(.+?)</[l]*>", 'Link.', result)
...
if not ( lstFetchedNews == ["No data"]):
    for news in lstFetchedNews:
        ...
        if title not in lstMasterNewsTitle:
            #Assign Category
            news["Category"] = assign_category(lstNewsCategories,news["content"])
            lstMasterNews.append(news)
            lstMasterNewsTitle.append(title)        
             
#Set Search Date Range
todayDate = date.today()
dtStartDate = todayDate - timedelta(days=60)
strToDate = str(str(todayDate.year) + "-" + str(todayDate.strftime("%m")) + "-" + str(todayDate.strftime("%d"))) + "T09:00:00Z"
strFromDate = str(str(dtStartDate.year) + "-" + str(dtStartDate.strftime("%m")) + "-" + str(dtStartDate.strftime("%d"))) + "T07:00:00Z"
...
for strGoodNewsQuery in lstCompiledQuery:
    for category in lstNewsCategories:
        ...
        for query in lstCategoryQuery:
            ...
            strURL = strSearchQueryURL + query +' AND '+ strGoodNewsQuery +"&lang=en&country=" + strCountry + "&from=" + strFromDate + "&to=" + strToDate + "@sortby=publishedAt&token=" + strCurKey
            time.sleep(1)
...
for news in lstMasterNews:
    try:
        ...
        if intCompScore > 0.5 and intPosScore > 0.5 and intNegScore < 0.2: 
            #Update dictionary
            ...
            dicGoodNews["Article_ImageURL"].append(news["image"])
